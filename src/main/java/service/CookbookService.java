...
public class CookbookService {
    ...
    public void setSelectedRecipe(
            ...
            TextField tf_level, TextField tf_meal, TextField tf_time, ListView tf_ingredients, ImageView img_image) {
        ...
        //przypisanie listy składików do LV
        ObservableList<Ingredient> ingredients = FXCollections.observableArrayList();
        ingredients.addAll(recipe.getIngredients());
        tf_ingredients.setItems(ingredients);
        //ustawienie obrazka
        img_image.setImage(new Image(recipe.getImagePath()));
    ...
    //okienko do uploadu pliku

    public String uploadFile(ImageView e_view){
        FileChooser fileChooser = new FileChooser();
        fileChooser.setTitle("Wybierz zdjęcie");
        fileChooser.setInitialDirectory(new File("C:\\SDA Workspace\\MyCoockbook\\src\\main\\resources\\img"));

        //filter typów obrazków
        fileChooser.getExtensionFilters().addAll(
                new FileChooser.ExtensionFilter("obrazy", "*.png","*.jpeg","*.jpg")
        );
        File file = fileChooser.showOpenDialog(new Stage());
        if(file != null) {
            String imagepath = file.getPath();
            String direct_path = "C:\\SDA Workspace\\MyCoockbook\\src\\main\\resources";
    ...
    public  void saveRecipe(TextField e_title, TextArea e_description, Spinner<Integer> e_time,
                            ...
                            ListView lv_ingredientsView, ComboBox e_ingriens, ImageView e_view){
        if(validRecipe(e_title, e_description, e_time, e_meal, e_level, e_type)){
            ...
            //zapisanie receptury do receptur
